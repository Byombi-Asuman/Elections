...
export const createPosition = async (req: AuthRequest, res: Response) => {
  try {
    const { electionId, title, description, maxWinners, order } = req.body;

    const position = await prisma.position.create({
      data: { electionId, title, description, maxWinners: parseInt(maxWinners) || 1, order: parseInt(order) || 0 },
    });
...
export const updatePosition = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const { title, description, maxWinners, order } = req.body;
